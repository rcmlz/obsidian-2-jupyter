...
```bash
cd obsidian-2-jupyter
```

If you replace `<your-name>` that should work now:

```bash
raku C:\Users\<your-name>\.raku\bin\from-markdown Exercises\some-exercise.md --flavor obsidian --to jupyter --default-language python --output Exercises\some-exercise.ipynb
```
...
Give it a whirl by executing `Tools/obsidian2ipynb` from the command line

```bash
Tools/obsidian2ipynb --verbose
```

or run the script via Raku explicitly

```bash
raku Tools/obsidian2ipynb --verbose
```
...
```bash
raku Tools/obsidian2ipynb --daemon --input-dir /path/to/your/vault
```
